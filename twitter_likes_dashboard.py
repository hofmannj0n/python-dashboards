...
df = pd.read_csv("https://raw.githubusercontent.com/DashBookProject/Plotly-Dash/master/Chapter-4/tweets.csv")
df["name"] = pd.Series(df["name"]).str.lower()
df["date_time"] = pd.to_datetime(df["date_time"], format="%d/%m/%Y %H:%M")  # Updated format
df = (
